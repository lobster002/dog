...
public class StringToNumber {
	...
	public static String bulidTextZHToALB(String text) {
		...
		while (m.find()) {
			...
			if (numZH.length() != 1 || numMap.containsKey(numZH)
					|| zhTen.equals(numZH)) {
	...
	private static String NumZHToALB(String numZH) {
		...
		for (String unitNum : unitNumMap.keySet()) {
			...
			if (index != -1) {
				numALB += NumZHToALB(numZH.substring(formIndex, index),
						unitNumMap.get(unitNum));
				formIndex = index + 1;
			}
		}
	...
	public static String NumberToString(int number) {
		String num_str = "";
		String[] strNum = String.valueOf(number).split("|");
		int nNumCount = strNum.length - 1;
		System.out.println(nNumCount);
		// String newStr = "";
